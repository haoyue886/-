  ...
  methods: {
    loadDataList: function () {
      ...
      let data = {
        ...
        recommended:
          that.dataForm.recommended == "" ? null : that.dataForm.recommended,
    ...
    expand: function (row, expqandedRows) {
      let that = this;
      if (expqandedRows.length > 0) {
        that.expand = [];
        that.expand.push(row.id);
        let data = {
          id: row.id,
        };
        //同步agios请求
        that.$http(
          "/doctor/searchContent",
          "POST",
          data,
          false,
          function (resp) {
            that.content.id = row.id;
            that.content.photo = `${that.$minioUrl}${
              resp.photo
            }?random=${Math.random()}`;
    ...
    updatePhotoSuccess: function () {
      this.content.photo = `${this.$minioUrl}/doctor/doctor-${
        this.content.id
